...
const fileUrl = (path, mode = "preview") => {
  ...
  const token = encodeURIComponent(
    localStorage.getItem("tour_admin_token") || "",
  );
...
const medicalSummary = (report) => {
  const details = [];
  if (report.referred_hospital_name)
    details.push(`Hospital: ${report.referred_hospital_name}`);
  if (report.medical_reference_no)
    details.push(`Ref: ${report.medical_reference_no}`);
  if (report.medical_reference_date)
    details.push(`Ref date: ${formatDate(report.medical_reference_date)}`);
  if (report.patient_name) details.push(`Patient: ${report.patient_name}`);
  if (report.patient_relation)
    details.push(`Relation: ${report.patient_relation}`);
  if (report.escort_employee_sap_id)
    details.push(`Escort SAP: ${report.escort_employee_sap_id}`);
  if (report.return_vehicle_required)
    details.push(`Return vehicle: ${report.return_vehicle_required}`);
  if (report.railway_availability)
    details.push(`Railway: ${report.railway_availability}`);
  if (report.leave_availed)
    details.push(`Leaves availed: ${report.leave_availed}`);
  if (report.leave_start_date || report.leave_end_date)
    details.push(
      `Leave dates: ${excelDate(report.leave_start_date)} to ${excelDate(report.leave_end_date)}`,
    );
  return details;
};
...
const employeeExcelColumns = [
  ...
  },
  {
    group: "Employee Details",
    label: "Designation",
    value: (report) => report.designation,
    width: 130,
  },
  {
    group: "Employee Details",
    label: "Grade",
    value: (report) => report.grade,
    width: 70,
  },
  {
    group: "Employee Details",
    label: "Department",
    value: (report) => report.department,
    width: 110,
  },
];
...
const reportDateExcelColumns = [
  {
    group: "Report Dates",
    label: "Created At",
    value: (report) => excelDate(report.created_at),
    width: 110,
  },
  {
    group: "Report Dates",
    label: "Modified At",
    value: (report) => excelDate(report.updated_at),
    width: 110,
  },
];
...
const approvalExcelColumns = [
  {
    ...
    value: (report) => report.approving_authority || "-",
    width: 170,
  },
  {
    group: "Approval Details",
  ...
  },
  {
    ...
    value: (report) => report.rejection_reason || "-",
    width: 220,
  },
];
...
const officialExcelColumns = [
  ...
  {
    group: "Official Tour Details",
    label: "Submitted Date",
    value: (report) => excelDate(reportDate(report)),
    width: 110,
  },
  {
    group: "Official Tour Details",
    label: "Purpose of Official Tour",
    value: (report) => report.purpose || "-",
    width: 190,
  },
  {
    group: "Official Tour Details",
    label: "Start Date",
    value: (report) => excelDate(report.start_date),
    width: 110,
  },
  {
    group: "Official Tour Details",
    label: "Trip Begins At",
    value: (report) => report.start_time || "-",
    width: 100,
  },
  {
    group: "Official Tour Details",
    label: "Trip Started From",
    value: (report) => report.start_place || "-",
    width: 150,
  },
  {
    group: "Official Tour Details",
    label: "End Date",
    value: (report) => excelDate(report.end_date),
    width: 110,
  },
  {
    group: "Official Tour Details",
    label: "Trip Ends At",
    value: (report) => report.end_time || "-",
    width: 100,
  },
  {
    group: "Official Tour Details",
    label: "Destination",
    value: (report) => report.destination || "-",
    width: 150,
  },
  {
    group: "Official Tour Details",
    label: "Mode of Travel",
    value: (report) => report.mode_of_travel || "-",
    width: 150,
  },
  {
    group: "Official Tour Details",
    label: "Weekly Off On",
    value: (report) => report.weekly_off || "-",
    width: 120,
  },
...
const selfMedicalExcelColumns = [
  ...
  ...reportDateExcelColumns,
  {
    group: "Self Medical Tour Details",
    label: "Submitted Date",
    value: (report) => excelDate(reportDate(report)),
    width: 110,
  },
  {
    ...
    value: (report) => report.referred_hospital_name || "-",
    width: 190,
  },
  {
    group: "Self Medical Tour Details",
    label: "Reference Letter No.",
    value: (report) => report.medical_reference_no || "-",
    width: 140,
  },
  {
    group: "Self Medical Tour Details",
    label: "Reference Letter Date",
    value: (report) => excelDate(report.medical_reference_date),
    width: 150,
  },
  {
    group: "Self Medical Tour Details",
  ...
  {
    group: "Self Medical Tour Details",
    label: "Any Leaves Availed In Between",
    value: (report) => report.leave_availed || "-",
    width: 180,
  },
  {
    group: "Self Medical Tour Details",
    label: "Leave Start Date",
    value: (report) => excelDate(report.leave_start_date),
    width: 130,
  },
  {
    group: "Self Medical Tour Details",
    label: "Leave End Date",
    value: (report) => excelDate(report.leave_end_date),
    width: 130,
  },
  ...approvalExcelColumns,
];
...
const escortDutyExcelColumns = [
  ...
  },
  {
    ...
    value: (report) => report.patient_name || "-",
    width: 150,
  },
  {
    group: "Escort Duty Details",
    label: "Relation With Patient",
    value: (report) => report.patient_relation || "-",
    width: 150,
  },
  {
    group: "Escort Duty Details",
    label: "Escort Employee SAP ID",
    value: (report) => report.escort_employee_sap_id || "-",
    width: 150,
  },
  {
    group: "Escort Duty Details",
  ...
  {
    group: "Escort Duty Details",
    label: "Employee's Weekly Off On",
    value: (report) => report.weekly_off || "-",
    width: 150,
  },
  {
...
const mixedExcelColumns = [
  ...
  },
  {
    group: "Report Summary",
    label: "Type of Tour",
    value: (report) => report.tour_type,
    width: 140,
  },
  {
    group: "Report Summary",
    label: "Main Purpose / Hospital",
    value: (report) => report.purpose || report.referred_hospital_name || "-",
    width: 220,
  },
  {
    group: "Report Summary",
    label: "Journey",
    value: (report) =>
      `${excelDate(report.start_date)} ${report.start_time || "-"} to ${excelDate(report.end_date)} ${report.end_time || "-"}`,
    width: 230,
  },
  {
    group: "Report Summary",
  ...
  },
  {
    ...
    value: (report) => medicalSummary(report).join(" | ") || "-",
    width: 320,
  },
  ...approvalExcelColumns,
];
...
const getExcelColumns = (reportList) => {
  ...
  if (types.length === 1 && types[0] === "Official")
    return officialExcelColumns;
  if (types.length === 1 && types[0] === "Medical(Self)")
    return selfMedicalExcelColumns;
  if (types.length === 1 && types[0] === "Medical (Escort Duty)")
    return escortDutyExcelColumns;
  return mixedExcelColumns;
};

const reportToExcelRow = (report, columns) =>
  columns.map((column) => column.value(report));
...
const fileLink = (filePath, label) => (
  ...
  >
    <span>{label}</span>
    <a href={fileUrl(filePath, "preview")} target="_blank" rel="noreferrer">
      Preview
    </a>
    <a href={fileUrl(filePath, "download")} target="_blank" rel="noreferrer">
      Download
    </a>
  </span>
);
...
const reportFiles = (report) => {
  const primaryFile =
    report.combined_pdf_path ||
    report.approval_note_path ||
    report.supporting_documents[0]?.file_path;
...
export default function AdminDashboard() {
  ...
  const downloadExcel = () => {
    ...
    const groupHeader = `<tr>${excelGroupHeaders(columns)
      .map(
        (group) =>
    ...
    const dataRows = reports
      .map(
        (report) =>
          `<tr>${reportToExcelRow(report, columns)
            .map((cell) => `<td>${excelValue(cell)}</td>`)
            .join("")}</tr>`,
      )
      .join("");
      ...
      <div className="shell">
                ...
                visibleReports.map((report) => (
                  <tr key={report.id}>
                    ...
                    <td>
                      ...
                      SAP: {report.sap_id}
                      <br />
                      {report.designation}, {report.grade}
                      <br />
                    ...
                    <td>
                      ...
                      <br />
                      {medicalSummary(report).map((item) => (
                        <span key={item}>
                          {item}
                          <br />
                        </span>
        ...
        {!loading && reports.length > 0 && (
          <div className="pagination-bar">
            <span>
              Showing {pageStart + 1}-
              {Math.min(pageStart + PAGE_SIZE, reports.length)} of{" "}
              {reports.length}
            </span>
        ...
        <div className="modal-backdrop">
          <form className="modal" onSubmit={submitRejection}>
            ...
            <p style={{ color: "#64748b", marginTop: 0 }}>
              {rejectTarget.name} - {rejectTarget.destination}
            </p>
            <textarea
              rows={4}
              value={rejectReason}
